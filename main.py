...
class KeySizeInput:
    def __init__(self, root):
        self.root = root
        self.root.title("Key Size Input")
        self.create_widgets()


    def create_widgets(self):
        tk.Label(self.root, text="Enter Key Size:").grid(row=0, column=0, sticky=tk.W)
        self.key_size_entry = tk.Entry(self.root)
        self.key_size_entry.grid(row=0, column=1)

        self.submit_button = tk.Button(self.root, text="Submit", command=self.submit_key_size)
        self.submit_button.grid(row=1, columnspan=2)

    def submit_key_size(self):
        key_size = int(self.key_size_entry.get())
        self.root.destroy()
        start_application(key_size)

def start_application(key_size):
...
if __name__ == "__main__":
    ...
    key_size = int(input("Enter the key size: "))  # Prompt user to enter key size
    root = tk.Tk()
    ...
    if existing_data:
        ...
        # Generate user keys based on the provided key size
        ...
        encrypted_data = blockchain.get_data(user1, existing_data, encrypted_data)
