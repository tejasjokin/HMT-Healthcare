import hyper_ledger.storage.helper
...
from hyper_ledger.plenum.common.constants import KeyValueStorageType, HS_LEVELDB, HS_ROCKSDB
...
class DbHashStore(HashStore):
    ...
    def open(self):
        self.nodesDb = hyper_ledger.storage.helper.initKeyValueStorage(
            self.db_type, self.dataDir, self.nodes_db_name,
            read_only=self._read_only, db_config=self.config.db_merkle_nodes_config)
        self.leavesDb = hyper_ledger.storage.helper.initKeyValueStorage(
