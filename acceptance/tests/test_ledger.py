from acceptance.tests.utils import pool_helper, wallet_helper, nym_helper, get_nym_helper, attrib_helper, get_attrib_helper,\
...
import pytest
import json
from hyper_ledger.indy import pool, did, ledger
from hyper_ledger.indy.error import  IndyError
import hashlib
import time
...
def chain(data, Block_size):
    ...
    class blockchain:
        ...
        def printBlockchain(self):
            ...
            # np.savetxt("Processed//Blockchain_data.txt", BCN_data, delimiter=',', fmt='%s')  # write data to csv file

    CEVcoin = blockchain()

    Block_data, data_index = [], []
    l = int(len(data))
    for n in range(Block_size):             # no. of blocks
        tem, tem_ind, i, j = [], [], n, 1
        while (j <= l):             # len(data)
            tem_ind.append(i)
            tem.append(data[i])     # data of each block
            if ((i + Block_size) < len(data)):
                i += Block_size     # next data in that block
            else:
                j += l  # last set
        j += 1
        Block_data.append(tem)      # add data to Block
        data_index.append(tem_ind)  # index of data in each block

    # sending data to get mined
    for i in range(Block_size):
        CEVcoin.mineBlock(Block_data[i])

    CEVcoin.printBlockchain()

    return Block_data, data_index
...
test_send_and_get_cred_def_negative()